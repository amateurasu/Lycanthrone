 ...
 * Currently the results are broken down into 10 buckets, from 0 millis to a quarter of a second, and a report is
 * emitted to the ProblemReporter every minute. If there are 0 entries for a given minute, it is not reported. Reports
 * aren't emitted until you call report().
 */
public class HistogramTracker {
    ...
    private final AtomicStampedReference<long[]> bars = new AtomicStampedReference<long[]>(
        new long[RANGES.length + 2], 0);
