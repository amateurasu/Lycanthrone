...
class DownloadControllerSpec extends Specification {
    ...
    def 'should return 404 and no content'() {
        expect:
        mockMvc.perform(get('/download/' + FileExamples.NOT_FOUND_UUID + "/file.txt"))
    ...
    def 'should send file if ETag present but not matching'() {
        expect:
        mockMvc.perform(get(TEXT_FILE).header(IF_NONE_MATCH, '"WHATEVER"'))
                .andExpect(status().isOk())
    }
    ...
    def 'should not send file if ETag matches content'() {
        ...
        expect:
        mockMvc.perform(get(TEXT_FILE).header(IF_NONE_MATCH, etag))
    ...
    def 'should not return file if wasn\'t modified recently'() {
        ...
        expect:
        mockMvc.perform(get(TEXT_FILE).header(IF_MODIFIED_SINCE, dateHeader))
                .andExpect(status().isNotModified())
    }
    ...
    def 'should return 200 OK on HEAD request, but without body'() {
        expect:
        mockMvc.perform(head(TEXT_FILE))
                .andExpect(status().isOk())
                .andExpect(content().bytes(new byte[0]))
    }
    ...
    def 'should return 304 on HEAD request if we have cached version'() {
        expect:
        mockMvc.perform(head(TEXT_FILE).header(IF_NONE_MATCH, FileExamples.TXT_FILE.getEtag()))
                .andExpect(status().isNotModified())
                .andExpect(header().string(ETAG, FileExamples.TXT_FILE.getEtag()))
    }
    ...
    def 'should return Content-length header'() {
        ...
        mockMvc.perform(head(TEXT_FILE))
                .andExpect(status().isOk())
                .andExpect(header().longValue(CONTENT_LENGTH, FileExamples.TXT_FILE.size))
    }
