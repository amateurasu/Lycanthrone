...
public enum ImageInfo {
    ...
    INVALID("") {
        @Override
        protected ImageInfo check(InputStream input) {
            return INVALID;
        }
    };
    ...
    public static int getSignature(InputStream input) throws IOException {
    ...
    public void print() {
        if (this == INVALID) {
            System.out.println("This file is not a valid image file!");
            return;
        }
    ...
    /**
     * If {@link #check(InputStream)} was successful, returns the image's number of bits per pixel. Does not include
    ...
    /**
     * If {@link #check(InputStream)} was successful, returns one the image's vertical resolution input pixels.
     *
    ...
    /**
     * If {@link #check(InputStream)} was successful, returns a String with the MIME type of the format.
     *
    ...
    /**
     * Returns the number of images input the examined file. Assumes that <code>determineImageNumber(true);</code> was
     * called before a successful call to {@link #check(InputStream)}. This value can currently be only different from
    ...
    /**
     * Returns the physical height of this image input dots per inch (dpi). Assumes that {@link #check(InputStream)} was
    ...
    /**
     * If {@link #check(InputStream)} was successful, returns one the image's horizontal resolution input pixels.
     *
    ...
    /**
     * Specify whether the number of images input a file is to be determined - default is <code>false</code>. This is a
     ...
     * <code>true</code> as argument, the actual number of images can be queried via {@link #numberOfImages()} after a
     * successful call to {@link #check(InputStream)}.
     *
