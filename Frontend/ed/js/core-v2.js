function Core() {
    const split = (input, i) => (input.search(" ") !== -1 ? input.split(" ") : input.match(new RegExp(`.{${i}}`, "g"))) || [input];
    const maps = [
        ['kh', 'x'],
        ['nh', 'n\''],
        ['c(?!h)', 'k'],
    ...
    this.b64 = {
        ...
        en: input => btoa(encodeURIComponent(input).replace(/%([0-9A-F]{2})/g, (match, p1) => String.fromCharCode(`0x${p1}`)))
    };
    this.uri = {
        id: "uri",
        name: "URI",
    ...
    this.vie = {
        ...
        de: () => null,
        en: input => maps.reduce((result, map) => input.replace(new RegExp(map[0], 'g'), map[1]).replace(new RegExp(map[0].toUpperCase(), 'g'), map[1].toUpperCase()), input)
    };
    this.uni = {
        id: "uni",
        name: "Unicode",
