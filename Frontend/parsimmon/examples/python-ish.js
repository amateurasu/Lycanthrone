...
const PyX = indent => P.createLanguage({
    /**
     * This is where the magic happens. Basically we need to parse a deeper indentation level on the first statement of
     * the block and keep track of new indentation level. Then we make a whole new set of parsers that use that new
     * indentation level for all their parsing. Each line past the first is required to be indented to the same level as
     * that new deeper indentation level.
     */
    Block: r => P.seqObj(
        P.string("block:"),
        r.NL,
        ...
        const {n, first} = args;
        return PyX(n).RestStatement.many().map(rest => ["BLOCK", first, ...rest]);
    }),
    ...
    IndentSame: r => r.CountSpaces.chain(n => n === indent
        ? P.of(n)
        : P.fail(`${n} spaces`)
    ),
    ...
    // parse state's desired indentation
    IndentMore: r => r.CountSpaces.chain(n => n > indent
        ? P.of(n)
        : P.fail(`more than ${n} spaces`)
    ),
