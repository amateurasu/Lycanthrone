...
function PyX(indent) {
    return P.createLanguage({
        /**
         This is where the magic happens. Basically we need to parse a deeper indentation level on the first statement
         of the block and keep track of new indentation level. Then we make a whole new set of parsers that use
         that new indentation level for all their parsing. Each line past the first is required to be indented to the
         same level as that new deeper indentation level.
         */
        Block: r =>
            P.seqObj(
                P.string("block:"),
        ...
        /* This is just a statement in our language. To simplify, this is either a block of code or just an identifier */
        Statement: r => P.alt(r.Block, r.Ident),

        /*
         This is a statement which is indented to the level of the current parse state. It's called RestStatement
         because the first statement in a block is indented more than the previous state, but the *rest* of the
        ...
        /* Count the current indentation level and assert it's equal to the current parse state's desired indentation */
