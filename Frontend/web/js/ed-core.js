const Core = () => {
    ...
    const regex_bin = /^(?:[01]{1,8}\s*)+$/;
    const regex_hex = /^(?:[0-9A-Fa-f]{2}\s*)+$/;
    const regex_b64 = /^(?:[\w+\/]{4})*(?:[\w+\/]{4}|[\w+\/]{3}=|[\w\/]{2}==)$/;
    const regex_uni = /^(?:(?:\\u[0-9a-fA-F]{4}|\w)?[ !'#$%&"()*+-.,\/\\:;<=>?@[\]^_`{|}~]?)*$/g;
    const regex_url = /^((https?|ftp):\/\/)?\w+([\-.]\w+)*\.[a-z0-9]{2,5}(:[0-9]{1,5})?(\/.*)?$/i;
    ...
    return {
        ...
        vie: {
            ...
            name: "New Vietnamese",
            valid: () => true,
            de: () => null, // "Bạn đùa à? Cái thứ này thì dịch thế đ** nào được!"
        ...
        decode: (core, input) => {
            ...
            return checkURI(result) ? decodeURI(result) : result
        },
