...
public class ArticleServlet extends HttpServlet {
    ...
    private void updateArticle(HttpServletRequest request, HttpServletResponse response) throws IOException {
        //language=SQL
        val sql = "UPDATE Submissions SET chosen = ? WHERE id = ?";

        val select = request.getParameter("select");
        val selectId = request.getParameter("select_id");
        val selected = select.equals("true") ? 1 : 0;
        val param = new Object[]{selected, selectId};
